...
macro_rules! sysinfo {
	( $(($sysinfo_fn:ident, $sysinfo_enum:ident);)* ) => {
		...
		pub mod r#async {
			use super::*;
			$(
				pub unsafe extern "C-unwind" fn $sysinfo_fn(lua: LuaState) -> i32 {
					lua.check_function(1);
					lua.push_value(1);
					let callback_ref = lua.reference();
					super::super::r#async::ASYNC_CONTROLLER.with(|controller| controller.borrow_mut().request(lua, super::super::r#async::SysInfoRequest {
						callback_ref,
