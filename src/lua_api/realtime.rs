...
use super::export::SysInfoResponseData;
use crate::sysinfo::RealtimeData;
...
const DAEMON_SCRIPT: &str = concat!(include_str!("realtime.lua"), '\0');

pub unsafe extern "C-unwind" fn refresh_async(lua: LuaState) -> i32 {
    super::export::r#async::realtime(lua)
}

pub unsafe extern "C-unwind" fn refresh(lua: LuaState) -> i32 {
    super::export::sync::realtime(lua)
}

pub unsafe extern "C-unwind" fn start(lua: LuaState) -> i32 {
    ...
    lua.load_string(
        std::ffi::CStr::from_bytes_with_nul_unchecked(DAEMON_SCRIPT.as_bytes()).as_ptr(),
    )
    .expect("Failed to start daemon script!");
    lua.call(0, 0);
    0
}

pub unsafe extern "C-unwind" fn stop(_lua: LuaState) -> i32 {
    0
}

pub unsafe extern "C-unwind" fn set_interval(lua: LuaState) -> i32 {
    start(lua);
    lua.get_global(lua_string!("serverstat"));
    lua.get_field(-1, lua_string!("realtime"));
...
macro_rules! initializer_func {
    ($func:ident) => {
        pub unsafe extern "C-unwind" fn $func(lua: LuaState) -> i32 {
            start(lua);
            super::export::sync::$func(lua)
        }
    };
}
...
pub unsafe extern "C-unwind" fn all(lua: LuaState) -> i32 {
    start(lua);
    refresh(lua)
}

pub unsafe extern "C-unwind" fn all_system(lua: LuaState) -> i32 {
    start(lua);
    refresh(lua);
...
pub unsafe extern "C-unwind" fn all_process(lua: LuaState) -> i32 {
    start(lua);
    refresh(lua);
